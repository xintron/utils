...
if [ $spotify -gt 1 ]; then
    playing=$(dbus-send --type=method_call --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Get string:"org.mpris.MediaPlayer2.Player" string:"PlaybackStatus" | sed -n '/variant/s/^.*"\([^"]\+\).*$/\1/p')
    if [ $playing == 'Playing' ]; then
        data="np: $(dbus-send --type=method_call --print-reply --dest=org.mpris.MediaPlayer2.spotify /org/mpris/MediaPlayer2 org.freedesktop.DBus.Properties.Get string:"org.mpris.MediaPlayer2.Player" string:"Metadata" | sed -n '/artist/ { n;n; h}; /title/ { n; H; g; s/^.*"\([^"]\+\).*\n.*"\([^"]\+\).*$/\1 - \2/p };')"
        if [ $# -gt 0 ] && [ $1 = 'space' ]; then
            echo " $data "
        else
            echo $data
